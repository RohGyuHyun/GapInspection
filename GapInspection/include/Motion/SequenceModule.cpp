		...
		case 2:	//CASE 2: Main sequence
			...
			switch (sub_seq_idx)
			{
				case 100:
					//READY INITIALIZE
					//Sleep(2000);
					strText.Format(_T("Ch No %d, Sequence Loop Start"), m_nSeqChannel + 1);
					m_pMotLog->WriteText(strText);
					m_SeqFlag.m_bInitialized = FALSE;
					...
					m_bReadyState = TRUE;
					m_bInspecting = FALSE;
					pDlgMain->SetSendPacket(m_nSeqChannel, READY);
					SetSeqIdx(prc_idx, seq_idx, 110);
					break;
				case 110://init
#ifdef PLC_USE
					if((pDlgMain->GetSeqStepIndex(m_nSeqChannel) == INIT_SEQ) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
#else
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == INIT_SEQ) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetBarcodeReadEnd(m_nSeqChannel))
#endif
					{
						m_bReadyState = FALSE;
						m_nSeqIdx = INIT_SEQ;
						strText.Format(_T("Ch No %d, Sequence Inspection Init Start(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						m_bSeqProgress = TRUE;
						m_nGrabStepIndex = 0;
						m_bSeqEnd = FALSE;
						...
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, INIT_SEQ);
					...
					break;
				case 120:
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == INIT_SEQ) && (!pDlgMain->GetSeqStepStart(m_nSeqChannel) && pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
					{
						strText.Format(_T("Ch No %d, Sequence Inspection Init End(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
				...
				case 130://grab 1
					...
					/*if(!pDlgMain->GetBarcodeString(m_nSeqChannel).Compare(_T("-")))
					{
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, INIT_SEQ);
						SetSeqIdx(prc_idx, seq_idx, 100);
					}*/
#ifdef PLC_USE
					if((pDlgMain->GetSeqStepIndex(m_nSeqChannel) == GRAB_SEQ) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
#else
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
#endif
					{
						pDlgMain->SetInspEndCheck(m_nSeqChannel);
						m_nSeqIdx = GRAB_SEQ + m_nGrabStepIndex;
						m_bInspecting = TRUE;
						strText.Format(_T("Ch No %d, Sequence Step 1 Grab Start(%S)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, m_nSeqIdx);
				...
				case 140:
					...
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ) && (!pDlgMain->GetSeqStepStart(m_nSeqChannel) && pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
					{
						strText.Format(_T("Ch No %d, Sequence Step 1 Grab End(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, GRAB_SEQ);
						pDlgMain->SetSeqStepStart(m_nSeqChannel, FALSE);
						pDlgMain->SetSeqStepEnd(m_nSeqChannel, TRUE);
...
#ifdef PLC_USE
					if((pDlgMain->GetSeqStepIndex(m_nSeqChannel) == GRAB_SEQ + 1) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetInspecting(m_nSeqChannel, GRAB_SEQ - 1))
#else
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ + 1) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetInspecting(m_nSeqChannel, GRAB_SEQ - 1))
#endif
					{
						//Sleep(500);
						m_nSeqIdx = GRAB_SEQ + m_nGrabStepIndex;
						strText.Format(_T("Ch No %d, Sequence Step 2 Grab Start(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, GRAB_SEQ + 1);
				...
				case 160:
					...
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ + 1) && (!pDlgMain->GetSeqStepStart(m_nSeqChannel) && pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetInspecting(m_nSeqChannel, GRAB_SEQ))
					{
						strText.Format(_T("Ch No %d, Sequence Step 2 Grab End(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, GRAB_SEQ + 1);
						pDlgMain->SetSeqStepStart(m_nSeqChannel, FALSE);
						pDlgMain->SetSeqStepEnd(m_nSeqChannel, TRUE);
...
#ifdef PLC_USE
					if((pDlgMain->GetSeqStepIndex(m_nSeqChannel) == GRAB_SEQ + 2) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetInspecting(m_nSeqChannel, GRAB_SEQ + 1))
#else
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ + 2) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))// && pDlgMain->GetInspecting(m_nSeqChannel, GRAB_SEQ + 1))
#endif
					{
						//Sleep(500);
						m_nSeqIdx = GRAB_SEQ + m_nGrabStepIndex;
						strText.Format(_T("Ch No %d, Sequence Step 3 Grab Start(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, GRAB_SEQ + 2);
				...
				case 180:
					...
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == GRAB_SEQ + 2) && (!pDlgMain->GetSeqStepStart(m_nSeqChannel) && pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
					{
						strText.Format(_T("Ch No %d, Sequence Step 3 Grab End(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, GRAB_SEQ + 2);
						pDlgMain->SetSeqStepStart(m_nSeqChannel, FALSE);
						pDlgMain->SetSeqStepEnd(m_nSeqChannel, TRUE);
				...
				case 190://end
					...
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == END_SEQ) && (pDlgMain->GetSeqStepStart(m_nSeqChannel) && !pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
					{
						strText.Format(_T("Ch No %d, Sequence Inspection Rslt Start(%s)"), m_nSeqChannel + 1, pDlgMain->GetBarcode(m_nSeqChannel));
						m_pMotLog->WriteText(strText);
						m_nSeqIdx = END_SEQ;
						m_bSeqEnd = TRUE;
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, END_SEQ);
						pDlgMain->SetInspEndCheckTimer(m_nSeqChannel);
				...
				case 200:
					...
					if((pDlgMain->GetSeqStep(m_nSeqChannel) == END_SEQ) && (!pDlgMain->GetSeqStepStart(m_nSeqChannel) && pDlgMain->GetSeqStepEnd(m_nSeqChannel)))
					{
						...
						pDlgMain->SetSeqStepIndex(m_nSeqChannel, -1);
						//pDlgMain->SetBarcode(m_nSeqChannel, _T("-"));
						m_nGrabStepIndex = 0;
						SetSeqIdx(prc_idx, seq_idx, 100);
						strText.Format(_T("Ch No %d, Sequence Inspection Rslt End"), m_nSeqChannel + 1);
						m_pMotLog->WriteText(strText);
						//Sleep(1500);
						strText.Format(_T("Ch No %d, Sequence Loop End"), m_nSeqChannel + 1);
						m_pMotLog->WriteText(strText);
