...
void CStaticMatDisplay::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if(m_bUseTracker && m_bActive)
	{
		AfxGetMainWnd()->SendMessage(USE_MSG_ZOOM_VIEW_TO_MAIN_DIALOG, ZOOM_VIEW_TO_MAIN_SET_TRACKER_IDX, nTrackerIdx);
	}
		...
		if(NULL != pos)
		{
			...
			switch(tracker->GetType())
			{
				...
				case 1:
					...
					if( m_lpImage->width < ( s_x + r_r * ((m_lZoomEndX - m_lZoomStartX) / (double)m_WinRect.Width()) ) )
					{
						r_r = ((double)(m_lpImage->width - s_x)) / ((m_lZoomEndX - m_lZoomStartX) / (double)m_WinRect.Width()) - 2;
					}
